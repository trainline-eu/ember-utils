var detectBrowser = function(userAgent, language) {
  ...
  */
  ...
  /**
    ...
    @default 'unknown'
  */
  browser.current = browser.msie ? 'msie' : browser.mozilla ? 'mozilla' : browser.chrome ? 'chrome' :
    browser.safari ? 'safari' : browser.opera ? 'opera' :
    browser.mobileSafari ? 'mobile-safari' : browser.android ? 'android' : 'unknown';
  return browser;
};
...
*/
Ember.browser = detectBrowser();
$(function() { $('html').addClass(Ember.browser.current); });
